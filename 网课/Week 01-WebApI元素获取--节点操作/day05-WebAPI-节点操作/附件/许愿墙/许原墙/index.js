    ...
    function cloneElement() {
        // 获取页面之中已经有的留言添加删除元素
        const tip1 = document.querySelectorAll('.tip1');
        // 单击关闭
        const closeC = document.querySelectorAll('.close');
        for (let i = 0; i < closeC.length; i++) {
            closeC[i].addEventListener("click", function () {
                tip1[i].style.display = "none";
            })
        }
        // 双击关闭
        const tip_h = document.querySelectorAll('.tip_h');
        for (let i = 0; i < tip_h.length; i++) {
            tip_h[i].addEventListener("dblclick", function () {
                tip1[i].style.display = "none";
            })
        }
    ...
    // 获取数据把索引给传入函数之中循环创建元素
    for (let i = 0; i < messages.length; i++) {
        createScrip(i);
    }

    // createScrip(messagesIndex);

    function createScrip(messagesIndex) {
        ...
        tip_h.title = "双击关闭纸条";
        // 双击关闭纸条
        tip_h.addEventListener("dblclick", function () {
        ...
        num.className = "num";
        // 填充对象id和time
        num.innerHTML = "第[" + messages[messagesIndex]["id"] + "]条 " + messages[messagesIndex]["time"];
        ...
        // 填充对象content
        tip_c.innerHTML = messages[messagesIndex]["content"];
        /* tip_c结束 */
        ...
        name.className = "name";
        // 填充对象name
        name.innerHTML = messages[messagesIndex]["name"];
