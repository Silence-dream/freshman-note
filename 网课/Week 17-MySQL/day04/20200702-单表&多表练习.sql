...
SELECT * FROM emp GROUP BY empno HAVING deptno<>10;

-- 2.	查询姓名第二个字母不是”A”且薪水大于800元的员工信息，按年薪降序排列

SELECT * FROM emp  WHERE ename NOT LIKE '_A%' AND sal>800 GROUP BY sal DESC;
...
SELECT e.*,a.a  FROM emp  AS e, (SELECT deptno,AVG(sal) AS a FROM emp GROUP BY deptno) AS a
WHERE e.deptno=a.deptno AND sal>a.a;
...
SELECT dept.dname,dept.loc,COUNT(emp.ename),AVG(sal) FROM emp,dept WHERE dept.deptno=emp.deptno GROUP BY emp.deptno;